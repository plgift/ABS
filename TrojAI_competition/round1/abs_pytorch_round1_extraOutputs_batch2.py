...
def main(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, modelName, ExperimentName, example_img_format='png',):
    ...
    if not os.path.exists(result_filepath+ '/'+ExperimentName+'/'+modelName): #when switching to batch, had to edit FROM model_filepath TO modelName
            ...
            os.makedirs(result_filepath+'/'+ExperimentName +'/'+modelName)
    ...
    if use_pickle:
            ...
            with open(result_filepath+'/'+ExperimentName+'/'+ modelName+'/reasr_info.pkl', 'wb') as f:
                pickle.dump(reasr_info, f)
            with open(result_filepath+'/'+ExperimentName+'/'+ modelName+'/max_reasr.pkl', 'wb') as f:
                pickle.dump(max_reasr, f)
            with open(result_filepath+'/'+ExperimentName+'/'+ modelName+'/config.pkl', 'wb') as f:
    ...
    if use_h5:
            ...
            with h5py.File('./'+ExperimentName +'/'+ modelName+'/reasr_info.h5', "w") as f:
                f.create_dataset('reasr_info', data=reasr_info)
            with h5py.File('./'+ExperimentName+'/'+ modelName+'/max_reasr.h5', "w") as f:
                f.create_dataset('max_reasr', data=maxreasr)
            with h5py.File('./'+ExperimentName+'/'+ modelName+'/config.h5', "w") as f:
                f.create_dataset('config', data=config)
            with h5py.File('./'+ExperimentName+'/'+ modelName+'/neuron_dict.h5', "w") as f:
...
if __name__ == "__main__":
    ...
    """
    --model_filepath /mnt/nfs/ml-shared/trojai_data/dl/round1train/models/[i]/model.pt 
    --result_filepath  /mnt/nfs/ml-shared/trojai_data/ABS/TAI_round1_abs/  
    --scratch_dirpath /mnt/nfs/ml-shared/trojai_data/ABS/TAI_round1_abs/   
    --examples_dirpath /mnt/nfs/ml-shared/trojai_data/dl/round1train/models/[i]/example_data/  
    --modelName id-00000001  
    --ExperimentName debugPTtest1 
    """
    ...
    ExperimentName='absPT_dense'
    for modelid in modelnums:
        model_filepath=os.path.join(nistbasepath,modelid,'model.pt')
        examples_dirpath=os.path.join(nistbasepath,modelid,'example_data')
        print(model_filepath)
        print(examples_dirpath)
        main(model_filepath,outputbasepath, outputbasepath, examples_dirpath, modelid, ExperimentName)
        #breakpoint()
        # try:
        #     print("helloooooo")
        #     print(i)#model number
        #     print(model_filepat,args.result_filepath, args.scratch_dirpath, args.examples_dirpath, args.modelName, args.ExperimentName)
